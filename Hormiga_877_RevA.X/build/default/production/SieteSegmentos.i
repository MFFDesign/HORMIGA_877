...
# 13 "./SieteSegmentos.h"
void DisplayBegin(char port,uint16_t *channels);
void DisplayPrint(char code, char disply, char dp,char dt);
# 2 "SieteSegmentos.c" 2
...
void DisplayBegin(char port,uint16_t *channels)
{
    ...
    {
        pinMode(*(ChPointer +i),0);
        digitalWrite(*(ChPointer +i),0);
    }
    switch(port)
    {
        case 'b':
...
void DisplayPrint(char code, char disply, char dp,char dt)
{
    ...
    if(dataPort == 'B')
    {
        if(dp == 1)
        {
            ...
            if(dt != 0xFF)
            {
                digitalWrite(disply,1);
                delay(dt);
                digitalWrite(disply,0);
                delay(dt);
            }
            else
            {
