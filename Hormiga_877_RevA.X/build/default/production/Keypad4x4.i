...
uint8_t KeypadBegin8TO15(const uint8_t ROWSNibble, const uint8_t COLSNibble, unsigned char *Keyclas)
{
    if((ROWSNibble == 0x16) || (ROWSNibble == 0x15))
    {
        if((COLSNibble == 0x16) || (COLSNibble == 0x15))
        {
            nRBPU = 0;
            RenROWS = ROWSNibble;
            COLumns = COLSNibble;
...
char KeypadRead(void)
{
    digitalWrite(*(ColPins),1);
    digitalWrite(*(ColPins + 1),0);
    digitalWrite(*(ColPins + 2),0);
    digitalWrite(*(ColPins + 3),0);
